...
class Recipe:
	...
	def get_recipe(id=0, name=""):
		...
		if not id and name:
			id = Recipe.db_obj.execute(f"SELECT id FROM recipes where name = '{name}'").fetchall()
			print(f"{id = }")
	...
	def add_to_meal_plan(id):
		...
		Recipe.db_obj.execute("INSERT INTO selected_meals(id) VALUES (?)", (id))
